...
#include "AutoLight.cginc"
	...
	#define FOG_DEPTH 1
			...
			//JUST VARIABLE
			sampler2D _MetallicMap,_SmoothnessMap;
            sampler2D _MainTex, _AlphaTex,_DetailTex,_TranslucencyMap,
            _EmissionMap;


			float4 _MainTex_ST, _DetailTex_ST,_TranslucencyMap_ST;
			float4 _Tint,_TranslucencyColor,_Emission;

			float _TranslucencyScale,_TranslucencyPower,_SSS;
...
float4 ApplyFog(float4 color, v2f i)
{
    ...
    return color;
			...
			//LIGHT AND INDIRECT LIGHT FUNCTION
    ...
    light.ndotl = DotClamped(i.normal, light.dir);
			...
			float4 TranslucentLightFrag(v2f i){
				float3 inversedNormal = i.normal;
				inversedNormal *= -1;
				UnityLight Light = CreateTranslucentLight (i,inversedNormal);

				float4 lightImpact = pow(tex2D(_TranslucencyMap, i.uv.xy) * Light.ndotl * float4(Light.color,1),2)*_TranslucencyColor;
				return lightImpact;
			}

			fixed4 NormalLightFrag(v2f i){
...
FragmentOutput frag(v2f i) : SV_Target
			{
				    ...
				    float4 TLight = TranslucentLightFrag(i);
				float4 finalLight = NormalLightFrag(i)+TLight + GetEmission(i);
